...
run_PA_bin_OR <- function(data, PA_bin_vars, fracture_var, confounders = NULL) {
  ...
  lapply(models, function(mod) {
    ...
    mutate(
      across(c(OR, `95% CI lower`, `95% CI upper`, `p-value`), ~ round(., 3))
    )
}
...
save_table_word(
  df = OR_all,  # the full table with all fractures and sexes
  table_number = "Table 3.9",
  folder_path = TABLES_DIR,
  title = "Adjusted Odds ratios for PA by fracture type sex combined"
)
