...
```{r}
cc_dat  <- readRDS(file.path(DATA_DERIVED, "cc_dat.Rds"))
...
cc_dat <- cc_dat %>%
  select (
    ...
    MET_mod = log_MET_mod, 
    MET_vig = log_MET_vig,
    MET_MVPA = log_MET_MVPA,
    MET_walk = log_MET_walk,
    summed_MET_all = log_summed_MET_all,
    date_assess_A0,
    assessment_country,
    age_A0,
    agegp_A0,
...
### Univariate regression with log of PA variable and overall fracture
```{r}
...
glm(cc_dat$SRF ~ cc_dat$MET_mod, family = binomial)
glm(cc_dat$SRF ~ cc_dat$MET_vig, family = binomial) 
glm(cc_dat$SRF ~ cc_dat$MET_walk, family = binomial) 
glm(cc_dat$SRF ~ cc_dat$MET_MVPA, family = binomial) 
glm(cc_dat$SRF ~ cc_dat$summed_MET_all, family = binomial) 
...
for (var in PA_vars) {
  ...
  models[[var]] <- glm(fmla, data = cc_dat, family = binomial)
}


univariable_OR_table <- lapply(models, function(mod) {
...
univariable_OR_table

save_table_word(
  df = univariable_OR_table,  # your data frame
  table_number = "Table 3.3",  # can be numeric or string
  folder_path = TABLES_DIR,       # uses setup-defined folder
  title = "Univariable OR table"
)

```

As above but transformed to z-score to standardise
